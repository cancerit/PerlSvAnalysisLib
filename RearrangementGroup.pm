...
sub new {
    ...
    for (@{$params{components}}) {
        if (!defined(Scalar::Util::blessed $_) || Scalar::Util::blessed($_) ne 'RearrangementConnectedComponent') {
            die "The 'components' argument in $class\->new() must be of type 'RearrangementConnectedComponent'";
        }
...
sub get_normalised_rg_patterns {
    # Permute the orders and orientations of the chromosomes such that the
    # resulting rearrangement pattern strings have the lowest lexicographic
    # order. 
    # 
    # This subroutine modifies $self directly and adds a value to key
    # 'best_patterns'. This key points to a hash with the following
    # keys:
    #     rg_string           - rearrangement string
    #     segs_per_chr_string - copy number string
    #     chr_ordering        - ordering of the chromosomes
    #     chr_is_inverted     - orientations of the chromosomes
    #     cn_changes          - copy number changes at the breakpoints
    #     cn_change_variances - variances of the copy number changes at the breakpoints
    # 
    ...
    for (0..$#permutations) {
        ...
        for (0..(2**scalar(@involved_chrs)-1)) {
            ...
            die if grep( { scalar($_) != 2 } @rg_buckets);
